...
import { ChevronLeft, ChevronRight, Check, Briefcase, DollarSign, Building2 } from 'lucide-react';
import authService from '../../services/auth.service';

const NEW_USER_STEPS = [
    { title: 'Account', icon: Building2 },
    { title: 'Company', icon: Briefcase },
...
const RegisterSponsor = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        contactEmail: '', phone: '', location: '', budgetRange: '', gender: '',
    });
    ...
    const validateStep = () => {
        ...
        if ((!isLoggedIn && step === 1) || (isLoggedIn && step === 0)) {
            if (!validateRequired(formData.companyName)) newErrors.companyName = 'Required';
        }
        setErrors(newErrors);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const payload = {
                ...
                contact_email: formData.contactEmail, phone: formData.phone,
                location: formData.location, budget_range: formData.budgetRange,
            ...
            if (isLoggedIn) {
                ...
                console.log('[RegisterSponsor] Logged-in payload:', JSON.stringify(payload, null, 2));
                const data = await authService.registerSponsor(payload);
                navigate(ROUTES.SPONSOR_DASHBOARD);
            } else {
            ...
            <div className="hidden lg:flex lg:w-1/2 relative bg-primary-900 overflow-hidden">
                ...
                <div className="relative z-10 p-12 xl:p-16 flex flex-col h-full text-white w-full">
                    <div className="flex flex-col gap-3">
                        ...
                        <h1 className="text-4xl xl:text-5xl font-extrabold mt-4 tracking-tight">Become a<br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-200 to-indigo-100">Sponsor</span></h1>
                        <p className="text-lg opacity-90 max-w-md font-medium mt-2">Support events, promote your brand, and connect with organisers on Qomrade.</p>
                    </div>
                    ...
                    <div className="w-full max-w-md mx-auto my-auto py-4">
                        <div className="text-center mb-6">
                            <h2 className="text-3xl font-bold text-primary mb-2">
                                {isLoggedIn ? 'Set Up Sponsor Profile' : 'Register as Sponsor'}
                            </h2>
                            <p className="text-secondary font-medium text-sm">
                                {isLoggedIn ? 'Add your sponsor details to start supporting events' : 'Set up your sponsor profile to start supporting events'}
                            </p>
                        </div>
                        ...
                        <form onSubmit={e => { e.preventDefault(); isLastStep ? handleSubmit() : handleNext(); }} className="space-y-4">
                            ...
                            {((!isLoggedIn && step === 1) || (isLoggedIn && step === 0)) && (
                                <>
                                    <Input label="Company Name" type="text" name="companyName" value={formData.companyName} onChange={handleChange} placeholder="Acme Corp" error={errors.companyName} required />
                                    <Input label="Industry" type="text" name="industry" value={formData.industry} onChange={handleChange} placeholder="Technology" />
                                    <div>
                                        <label className="block text-sm font-medium text-secondary mb-1">Description</label>
                                        <textarea name="description" value={formData.description} onChange={handleChange} rows={3} placeholder="Tell us about your company..." className="w-full px-4 py-2 bg-elevated border border-theme text-primary rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none resize-none" />
                                    </div>
                                    <Input label="Website" type="url" name="website" value={formData.website} onChange={handleChange} placeholder="https://acme.com" />
                                    <Input label="Contact Email" type="email" name="contactEmail" value={formData.contactEmail} onChange={handleChange} placeholder="sponsor@acme.com" />
                                </>
                            )}

                            {((!isLoggedIn && step === 2) || (isLoggedIn && step === 1)) && (
                                <>
                                    <Input label="Company Phone" type="tel" name="phone" value={formData.phone} onChange={handleChange} placeholder="+254700000000" />
                                    <Input label="Location" type="text" name="location" value={formData.location} onChange={handleChange} placeholder="Nairobi, Kenya" />
                                    <Input label="Budget Range" type="text" name="budgetRange" value={formData.budgetRange} onChange={handleChange} placeholder="e.g. $1k-$5k per event" />
                                    <div>
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                <Button type="submit" variant="primary" disabled={loading} className="flex-1">
                                    {loading ? 'Creating...' : isLastStep ? (
                                        isLoggedIn ? <>Create Sponsor Profile <Check className="w-4 h-4 ml-2" /></> : <>Create Sponsor Account <Check className="w-4 h-4 ml-2" /></>
                                    ) : (
