        ...
        <div className="min-h-screen bg-background py-8 px-4">
            <div className="max-w-4xl mx-auto">
                <div className="flex items-center gap-4 mb-6">
                    <button onClick={() => navigate('/tasks')} className="p-2 hover:bg-secondary/10 rounded-full text-primary">
                        <ArrowLeft size={24} />
                    </button>
                    <h1 className="text-2xl font-bold text-primary">Create Task</h1>
                </div>
                ...
                <div className="bg-elevated rounded-xl shadow-sm p-6 border border-theme">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div>
                            ...
                            <select value={formData.priority} onChange={(e) => setFormData({ ...formData, priority: e.target.value })} className="w-full px-4 py-2 border border-theme bg-elevated text-primary rounded-lg">
                        ...
                        <div>
                            ...
                            <input type="date" value={formData.due_date} onChange={(e) => setFormData({ ...formData, due_date: e.target.value })} className="w-full px-4 py-2 border border-theme bg-elevated text-primary rounded-lg" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-secondary mb-2">Due Time</label>
                            <input type="time" value={formData.due_time} onChange={(e) => setFormData({ ...formData, due_time: e.target.value })} className="w-full px-4 py-2 border border-theme bg-elevated text-primary rounded-lg" />
                        </div>
                    </div>

                    {/* Subtasks */}
                    <div className="mb-6">
                        <label className="block text-sm font-medium text-secondary mb-2">Subtasks</label>
                        <div className="flex gap-2 mb-2">
                            <input
                                ...
                                className="flex-1 px-4 py-2 border border-theme bg-elevated text-primary rounded-lg"
                                placeholder="Add a subtask..."
                            />
                            <button onClick={addSubtask} className="px-4 py-2 bg-primary text-white rounded-lg">
                                <Plus size={18} />
                            </button>
                        </div>
                        <ul className="space-y-2">
                            {subtasks.map((subtask, idx) => (
                                <li key={idx} className="flex items-center gap-2 p-2 bg-secondary/5 rounded-lg border border-theme">
                    ...
                    <div className="flex flex-wrap gap-3 justify-end border-t border-theme pt-6">
                        <button onClick={() => navigate('/tasks')} className="px-6 py-2 border border-theme rounded-lg text-primary hover:bg-secondary/5">Cancel</button>
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-elevated rounded-xl p-6 max-w-md w-full border border-theme">
                        ...
                        <div className="flex gap-3 justify-end">
                            <button onClick={() => setShowConfirmation(false)} className="px-4 py-2 border border-theme text-primary rounded-lg hover:bg-secondary/5">Cancel</button>
                            <button onClick={confirmSubmit} disabled={loading} className="px-4 py-2 bg-primary text-white rounded-lg">
